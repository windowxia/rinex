...
fn plot_title(observable: Observable) -> (String, String) {
    if observable.is_pseudorange_observable() {
        ("Pseudo Range".to_string(), "Pseudo Range".to_string())
    } else if observable.is_phase_observable() {
        ("Signal Phase".to_string(), "Phase".to_string())
    ...
    for observable in doris.observable().sorted() {
        ...
        for (station_index, station) in doris.doris_stations().sorted().enumerate() {
            if *observable == Observable::Temperature {
                ...
                let x = doris
                    .doris_rx_clock_offset()
                    .filter_map(|((t_i, _flag_i), station_i, _)| {
                        if station_i == station {
                            Some(t_i)
                        } else {
                            None
                ...
                let y = doris
                    .doris_rx_clock_offset()
                    .filter_map(|((_, _flag_i), station_i, value)| {
                        if station_i == station {
                            Some(value)
                        } else {
                            None
                ...
                let freq_label = Carrier::from_doris_observable(&observable.to_string())
                    .unwrap_or_else(|_| {
